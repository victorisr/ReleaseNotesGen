# .NET {ID-VERSION} installation instructions for macOS

The following instructions demonstrate installing .NET {ID-VERSION} on macOS. These instructions augment the more general [.NET install instructions](install.md), including installing with `.tar.gz` files, that work on multiple operating systems.
...
```bash
~# curl -Lo dotnet.tar.gz {MACOS-SDK-URL}
~# mkdir dotnet
